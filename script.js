...
// Decorative counter: starts at 993 and changes naturally by small amounts.
let fakeListeners = 993;
function updateFakeListeners() {
  const change = Math.floor(Math.random() * 7) - 3;
  fakeListeners = Math.max(980, Math.min(1010, fakeListeners + change));
  if (liveEl) liveEl.innerHTML = `<span></span> ${fakeListeners} भक्त अभी सुन रहे हैं`;
}
updateFakeListeners();
setInterval(updateFakeListeners, 12000 + Math.random() * 12000);
